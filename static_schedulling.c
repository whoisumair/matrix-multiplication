...
void matrix_multiply_parallel_static(double A[MAX_SIZE][MAX_SIZE], double B[MAX_SIZE][MAX_SIZE], double C[MAX_SIZE][MAX_SIZE], int N) {
    ...
    printf("Parallel Matrix Multiplication (Static Scheduling) Time: %.6f sec\n", end - start);
}
...
int main() {
    ...
    for (int t = 0; t < 3; t++) {
        ...
        for (int s = 0; s < 3; s++) {
            ...
            matrix_multiply_parallel_static(A, B, C, N);
        }
    }
